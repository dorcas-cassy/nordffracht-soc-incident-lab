...
> **Lab boundary:** All attack activity documented here is confined to an isolated, self-owned Docker lab. No third-party targets are involved. The target SSH port is accessible only on the Compose network; the Wazuh dashboard binds to `127.0.0.1:8443`. This repository does not yet include evidence from a completed run.
...
| --- | --- | --- | --- |
| 1. Recon | [T1595 Active Scanning](https://attack.mitre.org/techniques/T1595/) | `nmap` probes the target SSH service | Custom rule `100100` may flag an SSH pre-auth connection close. Correlate with the saved Nmap output; an endpoint agent does not see the complete port scan. |
...
Run the three commands in [the simulation guide](docs/attack-simulation.md), capture dashboard screenshots in `evidence/`, and replace bracketed run values in [the incident report](docs/incident-report.md). Screenshots are ignored by Git by default; remove the relevant ignore entry if you choose to publish sanitized evidence. To stop the stack, run `./scripts/lab.sh down`. `down -v` also deletes lab data and enrollment state.
...
| `docs/incident-report.md` | Pre-filled report, with run-specific fields to complete |
| `evidence/` | Place to keep local dashboard screenshots |
